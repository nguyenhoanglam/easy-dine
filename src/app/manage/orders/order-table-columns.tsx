...
const columns: ColumnDef<Order>[] = [
  {
    ...
    filterFn: (row, columnId, filterValue: string) => {
      return matchesSearchQuery(String(row.getValue(columnId)), filterValue);
    },
  },
  {
    id: "guestName",
    ...
    cell: function Cell({ row }) {
      const { ordersByGuestId } = useContext(OrderTableContext);
          ...
          {guest && (
            <Popover>
              ...
              <PopoverContent className="w-[320px] sm:w-[440px]">
                <OrderGuestDetail
                  guest={guest}
                  orders={ordersByGuestId[guest.id]}
                />
              </PopoverContent>
            </Popover>
          )}
  ...
  },
  {
    ...
    header: "Trạng thái",
    cell: function Cell({ row }) {
      const { changeOrderStatus } = useContext(OrderTableContext);

      return (
        <Select
          onValueChange={(value: (typeof OrderStatusValues)[number]) =>
            changeOrderStatus(row.original.id, {
              status: value,
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="space-y-2 text-sm">
        ...
        <div className="flex items-center space-x-4">
          {formatDate(row.original.updatedAt as unknown as string)}
        </div>
      </div>
    ),
  },
  {
    ...
    cell: function Actions({ row }) {
      const { setOrderIdToEdit: setOrderIdEdit } =
